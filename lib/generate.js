...
module.exports = async function generate(opts = {}) {
  ...
  // Read available custom toolchains from bare-make-toolchains.json
  const configPath = path.join(__dirname, '..', 'bare-make-custom-toolchains.json')
  let customToolchains = {}
  if (fs.existsSync(configPath)) {
    try {
      customToolchains = JSON.parse(fs.readFileSync(configPath, 'utf8'))
      // Format to the absolute path of this package by updating the values
      for (const key in customToolchains) {
        customToolchains[key] = path.join(__dirname, '..', 'toolchains', customToolchains[key])
      }
    } catch (err) {
      //TODO: handle missing custom toolchain file? or just silently ignore?
      console.warn('Error parsing custom toolchain file: ', err)
    }
  }

  // Merge with existing toolchains
  const allToolchains = { ...toolchains, ...customToolchains }
  if (target in allToolchains === false) {
    throw errors.UNKNOWN_TOOLCHAIN(`No toolchain found for target '${target}'`)
  }

  const args = ['-S', source, '-B', path.resolve(cwd, build), '-G', 'Ninja']

  args.push('--toolchain', allToolchains[target])
